...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      val leftTuple = (this.left, Left)
      val rightTuple = (this.right, Right)
      val upTuple = (this.up, Up)
      val downTuple = (this.down, Down)
      leftTuple :: (rightTuple :: (upTuple :: List(downTuple)))
    }
